...
use crate::tsx::svgr::async_bridge::{SvgWorkerPool, SvgRequest, SvgResponse, SvgBridgeError, create_svg_channels};
...
impl AsyncSvgProcessor {
    ...
    pub async fn process_vectors(
        ...
        info!("Starting intelligent SVG processing for {} vectors", total_requested);
        ...
        if !api_vectors.is_empty() {
            ...
            match batch_result {
                Ok(api_results) => {
                    ...
                    info!("✓ Batch processing completed successfully for {} API vectors", api_vectors.len());
                }
                Err(batch_error) => {
                    warn!("Batch processing failed: {}, falling back to individual processing", batch_error);
                    let individual_result = self.fallback_to_individual_processing(api_vectors, start_time).await?;
    ...
    fn should_use_api_for_vector(&self, vector: &VectorNode) -> bool {
        ...
        for path in fill_geometry {
            if path.path.len() > 200 {  // Arbitrary threshold for "complex" path
                return true;
            }
        }
    ...
    async fn try_batch_processing(
        ...
        for node_id in node_ids {
            ...
            if let Some(svg_result) = self.exporter.svg_cache.get(&cache_key) {
                ...
                return Err(SvgBridgeError::Channel(
                    format!("Vector {} not found in cache after batch prefetch", node_id)
                ));
            }
        }
    ...
    async fn fallback_to_individual_processing(
        ...
        // Collect responses
        let (svg_content, errors) = self.collect_responses(&mut response_rx, vectors.len()).await;
