use crate::conversion::svg::{AsyncSvgProcessor, SvgConfig};
use crate::core::config::{CodeGenConfig, CodeGenResult};
use crate::core::walker::Walker;
use crate::generators::tsx::generator::TsxGenerator;
use crate::generators::tsx::visitor::TsxVisitor;
...
impl UnifiedPipeline {
    ...
    pub async fn process_canvas(
        &mut self,
        canvas: &CanvasNode,
    ) -> crate::core::Result<CodeGenResult> {
    ...
    async fn generate_figma_tsx(&self, canvas: &CanvasNode) -> crate::core::Result<CodeGenResult> {
        ...
        if !tsx_visitor.pending_vectors().is_empty() && self.file_key.is_some() {
            info!(
                "Processing {} vectors with async SVG processor",
                tsx_visitor.pending_vectors().len()
            );
        ...
        // Generate code
        let codegen_system = crate::core::config::CodeGenSystem::new(
            tsx_visitor,
            tsx_generator,
    ...
    async fn process_vectors_async(
        &self,
        tsx_visitor: &mut crate::generators::tsx::visitor::TsxVisitor,
    ) -> crate::core::Result<()> {
        let file_key = self
            .file_key
            .as_ref()
        ...
        let mut processor = AsyncSvgProcessor::new(file_key.clone(), SvgConfig::default())
            .with_concurrency(10)
        ...
        for (node_id, svg_content) in result.svg_content {
            ...
            tsx_visitor
                .jsx_elements_mut()
                .insert(node_id.clone(), jsx_element);
    ...
    fn process_tsx_content(&mut self, content: &str) -> crate::core::Result<String> {
        ...
        let (cm, mut module) = etch_tsx::file::parse_tsx_file(temp_file.path())
            .map_err(|e| crate::core::Error::codegen(format!("Failed to parse TSX: {}", e)))?;
...
mod tests {
    use super::*;
    use crate::core::config::{SvgContainerMode, TextInSvgMode, VectorExportStrategy};
