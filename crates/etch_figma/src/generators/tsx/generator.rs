...
use crate::core::config::{CodeGenConfig, CodeGenResult, CodeGenerator};
...
pub struct TsxGenerator {
    ...
    pub(crate) include_react_imports: bool,
    ...
    pub(crate) exportable_only: bool,
    /// Path registry for extracting SVG paths (uses interior mutability)
    pub(crate) path_registry: RefCell<PathRegistry>,
    /// Configuration (optional, for advanced options)
    pub(crate) config: Option<CodeGenConfig>,
}
...
impl CodeGenerator<TsxVisitor> for TsxGenerator {
    fn generate(&self, visitor: &TsxVisitor) -> crate::core::Result<CodeGenResult> {
