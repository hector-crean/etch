...
use crate::conversion::svg::{FigmaSvgExporter, PathComplexity};
...
///
/// Note: This function is currently not used but kept for future async SVG export support
#[allow(dead_code)]
pub async fn create_vector_jsx_with_svg_export(
    vector: &VectorNode,
    exporter: &mut FigmaSvgExporter,
    file_key: &str,
) -> Result<JSXElement, crate::conversion::svg::exporter::SvgExportError> {
...
fn parse_svg_content_to_jsx(svg_content: &str) -> Vec<swc_ecma_ast::JSXElementChild> {
    ...
    match SvgParser::parse_svg_to_jsx(&cleaned_svg) {
        Ok(jsx_element) => {
            ...
            vec![swc_ecma_ast::JSXElementChild::JSXElement(Box::new(
                jsx_element,
            ))]
        }
        Err(e) => {
