use crate::codegen_ext::{CodeGenConfig, VectorExportStrategy};
...
mod tests {
    ...
    #[test]
    fn test_should_use_api_manual() {
        let config = VectorExportConfig {
            strategy: VectorExportStrategy::ManualConversion,
        ...
        assert!(!config.should_use_api(&vector));
    }

    #[test]
    fn test_should_use_api_figma_api() {
        let config = VectorExportConfig {
            strategy: VectorExportStrategy::FigmaApi,
            postprocess_enabled: false,
            fallback_to_manual: true,
        };
    ...
    fn test_should_postprocess() {
        ...
        assert!(config.should_postprocess());

        let config_no_postprocess = VectorExportConfig {
            strategy: VectorExportStrategy::Hybrid,
            postprocess_enabled: false,
            fallback_to_manual: true,
        };

        assert!(!config_no_postprocess.should_postprocess());
    }

    #[test]
    fn test_from_codegen_config() {
        ...
        let vector_config = VectorExportConfig::from(&codegen_config);

        assert!(matches!(
            vector_config.strategy,
            VectorExportStrategy::Hybrid
        ));
        assert!(vector_config.postprocess_enabled);
        assert!(vector_config.fallback_to_manual);
    }
}
